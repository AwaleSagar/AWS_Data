...
class ActionLanguageProcessor():
    ...
    def _maximumStrSimilarity(self,entity,keys):
        """
    ...
    def _take_action(self, interpretation):
        ...
        if location != None and appliance != None and action != None:
            ...
            and action.lower() in self.action_device_mapping[appliance]["actions"]): # We check the location exists
                ...
                return f"Okay, I have {action} the {appliance} in {location}"
    ...
    def _get_energy_consumption_timespan(self, interpretation):
        ...
        for entity in interpretation['entities'] :
            if entity['entity'] == "info":
                my_list.append(entity['value'])
            if entity['entity'] == "time" and entity['extractor'] == "DucklingHTTPExtractor":
                my_list.append(entity['text'])
        if my_list == []:
            return str("Sorry I was not able to retrive that information :( ")
        else:
            if len(my_list) == 1:
                enrg = collection.find_one({"address": int(142)})
                return str("Your {} is {} kwh".format(enrg['memory'],enrg['value']))
            else:
                if my_list[1] == 'today':
                    enrg = collection.find_one({"address": int(143)})
                    return str("Your {} is {} kwh".format(enrg['memory'],enrg['value']))
                if my_list[1] == 'yesterday':
                    enrg = collection.find_one({"address": int(147)})
                    return str("Your {} is {} kwh".format(enrg['memory'],enrg['value']))
                if my_list[1] == 'week' or my_list[1] == 'this week' :
                    enrg = collection.find_one({"address": int(144)})
                    return str("Your {} is {} kwh".format(enrg['memory'],enrg['value']))
                if my_list[1] == 'month' or my_list[1] == 'this month' :
                    enrg = collection.find_one({"address": int(145)})
                    return str("Your {} is {} kwh".format(enrg['memory'],enrg['value']))
                if my_list[1] == 'last day':
                    enrg = collection.find_one({"address": int(147)})
                    return str("Your {} is {} kwh".format(enrg['memory'],enrg['value']))
                if my_list[1] == 'last week':
                    enrg = collection.find_one({"address": int(148)})
                    return str("Your {} is {} kwh".format(enrg['memory'],enrg['value']))
                if my_list[1] == 'last month':
                    enrg = collection.find_one({"address": int(149)})
                    return str("Your {} is {} kwh".format(enrg['memory'],enrg['value']))
...
if __name__ == "__main__":
    ...
    # utterance = "give me an eco fact about heater"
    ...
    # utterance = "what is my energy consumption this week"
    # utterance = "can you give me air quality of last week ?"
    # utterance = "turn off the living room light"
    # utterance = "what is my energy consumption for today ?"
    # utterance = "can you give me air quality of last week ?"
    # utterance = "turn off the living room light"
